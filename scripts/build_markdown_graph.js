...
const docsSorted = docs.slice().sort((a, b) => (String(b.updated || '')).localeCompare(String(a.updated || '')) || a.title.localeCompare(b.title));
const generatedAt = docsSorted[0]?.updated || new Date().toISOString().slice(0, 10);
...
graph.nodeTypeCounts = graph.nodes.reduce((acc, node) => {
  const key = node.nodeType || 'unknown';
  acc[key] = (acc[key] || 0) + 1;
  return acc;
}, {});
graph.entityTypeCounts = graph.nodes.filter(node => node.nodeType === 'entity').reduce((acc, node) => {
  const key = node.entityType || 'Unspecified';
  acc[key] = (acc[key] || 0) + 1;
  return acc;
}, {});

const docsForJson = docsSorted.map(doc => ({
  id: doc.id,
  slug: doc.slug,
  title: doc.title,
  kind: doc.kind,
...
fs.writeFileSync(path.join(docsDataDir, 'documents.json'), JSON.stringify({ generatedAt, documents: docsForJson }, null, 2) + '\n');

const mmd = ['graph TD', ...graph.edges.map(edge => {
  const from = nodes.get(edge.from);
  const to = nodes.get(edge.to);
  return `  ${edge.from.replace(/[^A-Za-z0-9]/g, '_')}["${String(from.label).replace(/"/g, '\\"')}"] -->|${edge.type}| ${edge.to.replace(/[^A-Za-z0-9]/g, '_')}["${String(to.label).replace(/"/g, '\\"')}"]`;
})].join('\n') + '\n';
fs.writeFileSync(path.join(graphDir, 'graph.mmd'), mmd);
fs.writeFileSync(path.join(docsDataDir, 'graph.mmd'), mmd);

const topDocs = docsSorted.map(d => ({
  path: d.path,
  title: d.title,
  kind: d.kind,
...
const summary = [
  ...
  '## Node types',
  '',
  ...Object.entries(graph.nodeTypeCounts).sort((a, b) => a[0].localeCompare(b[0])).map(([k, v]) => `- ${k}: ${v}`),
  '',
  '## Entity types',
  '',
  ...Object.entries(graph.entityTypeCounts).sort((a, b) => a[0].localeCompare(b[0])).map(([k, v]) => `- ${k}: ${v}`),
  '',
